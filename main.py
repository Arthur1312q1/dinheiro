...
══════════════════════════════════════════════════════════════════════
FIX v18 — Saídas 100% alinhadas com backtest (sem monitor contínuo)
══════════════════════════════════════════════════════════════════════
...
- Refresh de cache simplificado (apenas preço de mercado)
══════════════════════════════════════════════════════════════════════
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# LIVE TRADER (VERSÃO FINAL – SEM MONITOR, SAÍDAS VIA ESTRATÉGIA)
# ═══════════════════════════════════════════════════════════════════════════════
class LiveTrader:
    def __init__(self):
        ...
        for act in actions:
            ...
            # ── EXIT SHORT ────────────────────────────────────────────
                ...
                if pos and pos['side'] == 'short':
                    qty = pos['size']
                    if 'price' not in act:
                        log.error("  ❌ EXIT_SHORT sem preço na ação!")
                        continue
                    exit_price = act['price']
                    reason = act.get('exit_reason', 'EXIT')
    ...
    def _refresh_cache(self):
        # Apenas atualiza o preço de mercado; posição e saldo vêm da estratégia
        px = self._mark_price()
        if px > 0:
            self._cache_px = px
...
# ═══════════════════════════════════════════════════════════════════════════════
# FLASK DASHBOARD (IDÊNTICO AO ORIGINAL – NÃO HOUVE ALTERAÇÕES)
# ═══════════════════════════════════════════════════════════════════════════════
app      = Flask(__name__)
...
DASH = """<!DOCTYPE html>
... (dashboard HTML - mantido igual ao original para economia de espaço)
..."""
